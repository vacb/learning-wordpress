...
class MyNotes {
  ...
  editNote(e) {
    let thisNote = jquery__WEBPACK_IMPORTED_MODULE_0___default()(e.target).parents("li"); // First run, the attribute won't exist so will always be false. After that need to set the attribute to 'editable'
...
class Search {
  ...
  openOverlay() {
    ...
    jquery__WEBPACK_IMPORTED_MODULE_0___default()("body").addClass("body-no-scroll"); // Empty out search field if you open the overlay a second time
  ...
  getResults() {
    jquery__WEBPACK_IMPORTED_MODULE_0___default.a.getJSON( // Custom route called 'search' and 'term' specified in search-route.php
    universityData.root_url + "/wp-json/university/v1/search?term=" + this.searchField.val(), results => {
      this.resultsDiv.html(`
        <div class="row">
          <div class="one-third">
            <h2 class="search-overlay__section-title">General Information</h2>
            ${results.generalInfo.length ? '<ul class="link-list min-list">' : "<p>Your search returned no results in this category.</p>"}
          ...
          <div class="one-third">
            ...
            <h2 class="search-overlay__section-title">Programs</h2>
              ${results.programs.length ? '<ul class="link-list min-list">' : `<p>Your search returned no results. <a href="${universityData.root_url}/programs">View all programs</a></p>`}
              ${results.programs.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
              ${results.programs.length ? "</ul>" : ""}
            
            <h2 class="search-overlay__section-title">Academics</h2>

            ${results.academics.length ? '<ul class="academic-cards">' : `<p>Your search returned no results.</p>`}
          ...
          <div class="one-third">
            ...
            ${results.campuses.length ? '<ul class="link-list min-list">' : `<p>Your search returned no results. <a href="${universityData.root_url}/campuses">View all campuses</a></p>`}
            ${results.campuses.map(item => `<li><a href="${item.permalink}">${item.title}</a></li>`).join("")}
            ${results.campuses.length ? "</ul>" : ""}

            <h2 class="search-overlay__section-title">Events</h2>

            ${results.events.length ? "" : `<p>Your search returned no results. <a href="${universityData.root_url}/events">View all events</a></p>`}
